...
const EditBrandModal = ({ brand, onClose, onSuccess }) => {
  ...
  const submit = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin/brands/${brand._id}`, {
        method: "PUT",
        headers: {
        ...
        <div className="px-8 py-6 grid grid-cols-2 gap-x-6 gap-y-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="h-[44px] px-4 py-2 rounded-lg border border-gray-300 text-[14px] bg-white focus:outline-none"
            />
          </div>
        </div>
