// src/components/models/AddModelModal.jsx
import { X } from "lucide-react";
import { useEffect, useState } from "react";
import { API_URL } from "../../config/api";

const AddModelModal = ({ onClose, onSuccess }) => {
  const [brands, setBrands] = useState([]);
  const [form, setForm] = useState({ brand: "", name: "", status: "active" });
    ...
    <div onClick={onClose} className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div onClick={(e) => e.stopPropagation()} className="bg-white w-[560px] rounded-2xl relative">
        <div className="flex items-center justify-between px-8 pt-6">
          <h2 className="text-[22px] font-semibold text-gray-900">Добавить модель</h2>
        ...
        <div className="px-8 py-6 grid grid-cols-2 gap-x-6 gap-y-4">
          <div className="flex flex-col gap-1 col-span-2">
            <label className="text-[13px] font-medium text-gray-700">Марка</label>
            <select value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} className="h-[44px] px-4 rounded-lg border border-gray-300 text-[14px] bg-white focus:outline-none">
          ...
          <div className="flex flex-col gap-1 col-span-2">
            <label className="text-[13px] font-medium text-gray-700">Название модели</label>
        ...
        <div className="flex justify-end gap-3 px-8 pb-6">
          ...
          <button onClick={submit} disabled={loading || !form.brand || !form.name.trim()} className="h-[44px] px-6 rounded-lg bg-[#32BB78] text-white text-[14px] font-medium hover:bg-[#28a96a] transition flex items-center justify-center">
