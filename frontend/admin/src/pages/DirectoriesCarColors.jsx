// src/pages/DirectoriesCarColors.jsx
...
const COLORS = Array.from({ length: 40 }, (_, i) => {
  const palette = [
    ...
    { hex: "#1E90FF", name: "Синий" },
  ];
  const c = palette[i % palette.length];
  return {
    id: `color-${String(i + 1).padStart(3, "0")}`,
    hex: c.hex,
    name: c.name,
...
const DirectoriesCarColors = () => {
  ...
  const visible = COLORS.slice(start, end);
            ...
            <table className="w-full min-w-[1000px] text-[13px]">
              <thead className="bg-gray-50 text-gray-500">
                <tr>
                  ...
                  <th className="px-3 py-3 w-[160px] text-left">HEX</th>
                  <th className="px-3 py-3 w-[220px] text-left">Название</th>
              ...
              <tbody>
                {visible.map((c, i) => (
                  <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50 transition">
                    ...
                    <td className="px-3 py-3 text-gray-600">{c.id}</td>
                    <td className="px-3 py-3">
                      <div className="h-6 w-6 rounded-full border border-gray-300" style={{ backgroundColor: c.hex }} />
                    </td>
